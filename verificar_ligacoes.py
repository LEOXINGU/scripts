...
if moldA.name()=='aux_moldura_a' and moldB.name()=='aux_moldura_a' and nomeA != nomeB:
    ...
    poligonos = ['adm_area_pub_civil_a', 'adm_area_pub_militar_a', 'adm_descontinuidade_geometrica_a', 'adm_edif_pub_civil_a', 'adm_edif_pub_militar_a', 'adm_posto_fiscal_a', 'adm_posto_pol_rod_a', 'asb_area_abast_agua_a', 'asb_area_saneamento_a', 'asb_cemiterio_a', 'asb_dep_abast_agua_a', 'asb_dep_saneamento_a', 'asb_descontinuidade_geometrica_a', 'asb_edif_abast_agua_a', 'asb_edif_saneamento_a', 'eco_area_comerc_serv_a', 'eco_area_ext_mineral_a', 'eco_area_industrial_a', 'eco_deposito_geral_a', 'eco_descontinuidade_geometrica_a', 'eco_edif_agrop_ext_veg_pesca_a', 'eco_edif_comerc_serv_a', 'eco_edif_ext_mineral_a', 'eco_edif_industrial_a', 'eco_equip_agropec_a', 'eco_ext_mineral_a', 'eco_plataforma_a', 'edu_area_ensino_a', 'edu_area_lazer_a', 'edu_area_religiosa_a', 'edu_area_ruinas_a', 'edu_arquibancada_a', 'edu_campo_quadra_a', 'edu_coreto_tribuna_a', 'edu_descontinuidade_geometrica_a', 'edu_edif_const_lazer_a', 'edu_edif_const_turistica_a', 'edu_edif_ensino_a', 'edu_edif_religiosa_a', 'edu_piscina_a', 'edu_ruina_a', 'enc_area_comunicacao_a', 'enc_area_energia_eletrica_a', 'eco_area_agrop_ext_veg_pesca_a', 'enc_descontinuidade_geometrica_a', 'enc_edif_comunic_a', 'enc_edif_energia_a', 'enc_est_gerad_energia_eletr_a', 'enc_grupo_transformadores_a', 'enc_hidreletrica_a', 'enc_termeletrica_a', 'enc_zona_linhas_energia_com_a', 'hid_area_umida_a', 'hid_bacia_hidrografica_a', 'hid_banco_areia_a', 'hid_corredeira_a', 'hid_foz_maritima_a', 'hid_ilha_a', 'hid_natureza_fundo_a', 'hid_quebramar_molhe_a', 'hid_queda_dagua_a', 'hid_recife_a', 'hid_reservatorio_hidrico_a', 'hid_rocha_em_agua_a', 'hid_terreno_suj_inundacao_a', 'lim_area_de_litigio_a', 'lim_area_desenv_controle_a', 'lim_area_especial_a', 'lim_area_particular_a', 'lim_area_politico_adm_a', 'lim_area_uso_comunitario_a', 'lim_bairro_a', 'lim_descontinuidade_geometrica_a', 'lim_distrito_a', 'lim_municipio_a', 'lim_outras_unid_protegidas_a', 'lim_pais_a', 'lim_regiao_administrativa_a', 'lim_sub_distrito_a', 'lim_terra_indigena_a', 'lim_terra_publica_a', 'lim_unidade_conserv_nao_snuc_a', 'lim_unidade_federacao_a', 'lim_unidade_protecao_integral_a', 'lim_unidade_uso_sustentavel_a', 'loc_area_habitacional_a', 'loc_area_urbana_isolada_a', 'loc_descontinuidade_geometrica_a', 'loc_edif_habitacional_a', 'loc_edificacao_a', 'loc_hab_indigena_a', 'pto_area_est_med_fenom_a', 'pto_descontinuidade_geometrica_a', 'pto_edif_constr_est_med_fen_a', 'rel_descontinuidade_geometrica_a', 'rel_dolina_a', 'rel_duna_a', 'rel_elemento_fisiog_natural_a', 'sau_area_saude_a', 'sau_area_servico_social_a', 'sau_descontinuidade_geometrica_a', 'sau_edif_saude_a', 'sau_edif_servico_social_a', 'tra_area_duto_a', 'tra_area_estrut_transporte_a', 'tra_atracadouro_a', 'tra_descontinuidade_geometrica_a', 'tra_eclusa_a', 'tra_edif_constr_aeroportuaria_a', 'tra_edif_constr_portuaria_a', 'tra_edif_metro_ferroviaria_a', 'tra_edif_rodoviaria_a', 'tra_faixa_seguranca_a', 'tra_fundeadouro_a', 'tra_local_critico_a', 'tra_obstaculo_navegacao_a', 'tra_patio_a', 'tra_posto_combustivel_a']
                ...
                if buffA.intersects(buffB):
                    ...
                    if not(nomeA == nomeB):
                        ...
                        if not(attA[1:min(len(attA), len(attB))-1] == attB[1:min(len(attA), len(attB))-1]):
                            # Problema 2
                            texto = 'O(s) atributo(s) '
                            for k in range(1, min(len(attA), len(attB))-1):
                                if attA[k] != attB[k]:
                                    campo = dict_fields[nomeA][k]
                                    texto += '"' +campo +'", '
                            texto = texto[:-2] + ' nao confere(m)'
                            att = [nomeA+': ' + str(attA[0]) + ' e ' + str(attB[0]) , 'Atributo(s) nao confere(m)', texto]
                            geom = buffA.intersection(buffB)
                            c = geom.centroid()
                            feature.setGeometry(c)
                            feature.setAttributes(att)
